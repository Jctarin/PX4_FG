...
Vector_declinacion= np.array([[ 47, 46, 45, 43, 42, 41, 39, 37, 33, 29, 23, 16, 10, 4, -1, -6, -10, -15, -20, -27, -34, -42, -49, -56, -62, -67, -72, -74, -75, -73, -61, -22, 26, 42, 47, 48, 47 ],
...
Vector_inclinacion=np.array([[ -78, -76, -74, -72, -70, -68, -65, -63, -60, -57, -55, -54, -54, -55, -56, -57, -58, -59, -59, -59, -59, -60, -61, -63, -66, -69, -73, -76, -79, -83, -86, -87, -86, -84, -82, -80, -78 ],
...
Vector_fuerzas=np.array([[ 62, 60, 58, 56, 54, 52, 49, 46, 43, 41, 38, 36, 34, 32, 31, 31, 30, 30, 30, 31, 33, 35, 38, 42, 46, 51, 55, 59, 62, 64, 66, 67, 67, 66, 65, 64, 62 ],
...
def Constrain(valor,min_val,max_val):
    if valor < min_val: return min_val
    if valor > max_val: return max_val
    return valor

def Indice_tabla(valor,min_val,max_val):
    valor=Constrain(valor,min_val,max_val)
    valor=abs((-(min_val)+valor)/SAMPLING_RES)
    return valor

def Valor_tabla(lat,lon,tabla):
    if lat<-90 or lat>90 or lon<-180 or lon>180:
        return 0
    else:
        lat_min=int((lat/SAMPLING_RES)*SAMPLING_RES)
        lon_min=int((lon/SAMPLING_RES)*SAMPLING_RES)

        lat_min_indice=int(Indice_tabla(lat_min,SAMPLING_MIN_LAT,SAMPLING_MAX_LAT))
        lon_min_indice=int(Indice_tabla(lon_min,SAMPLING_MIN_LON,SAMPLING_MAX_LON))

        dato_sw=tabla[lat_min_indice][lon_min_indice]
        dato_se=tabla[lat_min_indice][lon_min_indice+1]
        dato_ne=tabla[lat_min_indice+1][lon_min_indice+1]
        dato_nw=tabla[lat_min_indice][lon_min_indice]

        lat_escala=Constrain((lat-lat_min)/SAMPLING_RES,0,1)
        lon_escala = Constrain((lon - lon_min) / SAMPLING_RES, 0, 1)
    
        data_min=lon_escala*(dato_se-dato_sw)+dato_sw
        data_max=lon_escala*(dato_ne-dato_nw)+dato_nw
        
        result=lat_escala*(data_max-data_min)+data_min
        
        return result


def Declinacion_magnetica(lat,lon):
    return Valor_tabla(lat,lon,Vector_declinacion)

def Inclinacion_magnetica(lat,lon):
    return Valor_tabla(lat,lon,Vector_inclinacion)

def Fuerza_magnetica(lat,lon):
    return Valor_tabla(lat,lon,Vector_fuerzas)